  ...
  <div ref="scrollRef" class="flex-1 overflow-y-auto px-4 py-4 space-y-3">
    <div v-for="(msg, i) in messages" :key="i"
      ...
      <div :class="['max-w-[85%] text-sm leading-relaxed',
        msg.role === 'user'
          ? 'bg-copper-500 text-white rounded-2xl rounded-tr-md p-3'
        ...
        <div v-if="msg.steps && msg.steps.length > 0" class="mb-2">
          <div v-for="(step, si) in msg.steps" :key="si"
            class="flex items-center gap-1.5 text-xs text-warm-500 mb-0.5">
            ...
            <template v-else-if="step.type === 'tool_call'">
              <svg class="w-3 h-3 text-copper-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
              <span>调用工具：<code class="font-mono text-copper-600 bg-copper-500/10 px-1 rounded">{{ step.name }}</code></span>
            </template>
            <template v-else-if="step.type === 'tool_result'">
              ...
              <span><code class="font-mono text-moss-600 bg-moss-500/10 px-1 rounded">{{ step.name }}</code> 执行完毕</span>
            </template>
          </div>
        </div>
        <!-- 消息内容 -->
        <div :class="[msg.role === 'user' ? '' : msg.steps && msg.steps.length ? '' : 'p-3']">
          ...
          <span v-if="msg.streaming" class="inline-block w-1.5 h-4 bg-copper-400 animate-pulse ml-0.5 align-text-bottom"></span>
        </div>
      </div>
    </div>
