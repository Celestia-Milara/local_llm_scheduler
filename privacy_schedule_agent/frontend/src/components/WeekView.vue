  ...
  <div class="flex-1 flex flex-col overflow-hidden min-h-0">
    ...
    <div class="flex items-center gap-3 px-5 pt-5 pb-3 shrink-0">
      ...
      <div class="flex items-center gap-2">
        <button @click="prevWeek" class="p-1 text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors rounded-lg hover:bg-surface-100 dark:hover:bg-zinc-800">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
          </svg>
        </button>
        <h2 class="font-display text-base font-semibold text-zinc-800 dark:text-zinc-200 w-52 text-center">{{ weekLabel }}</h2>
        <button @click="nextWeek" class="p-1 text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors rounded-lg hover:bg-surface-100 dark:hover:bg-zinc-800">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
    ...
    <div class="grid grid-cols-[56px_repeat(7,1fr)] px-5 pb-2 shrink-0">
      ...
      <div v-for="d in weekDaysList" :key="d.dateKey" class="text-center py-1"
        :class="d.dateKey === todayKey ? 'bg-primary-500/10 dark:bg-primary-500/20 rounded-lg' : ''">
        <div class="text-xs text-zinc-400 dark:text-zinc-500 font-bold">{{ d.weekName }}</div>
        <div class="font-display text-sm font-semibold leading-tight"
          :class="d.dateKey === todayKey ? 'text-primary-500' : 'text-zinc-700 dark:text-zinc-200'">
          {{ d.day }}
        </div>
      </div>
    </div>
    ...
    <div v-if="allDayEvents.length > 0" class="px-5 pb-2 shrink-0">
      <div class="flex items-center gap-1 mb-1">
        <span class="text-[9px] text-zinc-400 font-medium uppercase tracking-wider">全天</span>
        <div class="flex-1 h-px bg-zinc-200/40"></div>
      </div>
    ...
    <div ref="scrollRef" class="flex-1 overflow-y-auto min-h-0 px-5 pb-5">
      <div ref="gridRef" class="relative rounded-xl border border-zinc-200/60 dark:border-zinc-700/60 bg-white dark:bg-zinc-900 min-h-full">
        <div class="relative" style="height: 1440px">
          ...
          <div v-for="idx in weekendIndices" :key="'we-' + idx"
            class="absolute top-0 bottom-0 bg-surface-50 dark:bg-zinc-800/50 pointer-events-none"
            :style="{ left: `calc(56px + (100% - 56px) * ${idx} / 7)`, width: `calc((100% - 56px) / 7)` }">
          </div>

          <!-- Today column highlight -->
          <div v-if="todayColIndex >= 0"
            class="absolute top-0 bottom-0 bg-primary-400/8 dark:bg-primary-400/15 pointer-events-none"
            :style="{ left: `calc(56px + (100% - 56px) * ${todayColIndex} / 7)`, width: `calc((100% - 56px) / 7)` }">
          </div>

          <!-- Hour lines -->
          <div v-for="h in 24" :key="h"
            class="absolute left-0 right-0 border-t border-zinc-200/50 dark:border-zinc-700/50"
            :style="{ top: h * 60 + 'px' }">
            <span class="hidden sm:block absolute left-0 -top-2.5 w-12 text-right pr-3 text-[11px] text-zinc-400 dark:text-zinc-500 font-semibold select-none">
          ...
          <div v-for="h in 24" :key="'m-' + h"
            class="absolute left-0 right-0 border-t border-zinc-100 dark:border-zinc-800/80"
            :style="{ top: h * 60 + 30 + 'px' }"></div>

          <!-- Vertical day separators -->
          <div v-for="i in 7" :key="'c-' + i"
            class="absolute top-0 bottom-0 border-l border-zinc-200/40 dark:border-zinc-700/40 pointer-events-none"
            :style="{ left: `calc(56px + (100% - 56px) * ${i - 1} / 7)`, width: `calc((100% - 56px) / 7)` }">
          </div>

          <!-- Drop target indicator during drag -->
          <div v-if="dragActive"
            class="absolute z-20 pointer-events-none border-t-2 border-dashed border-primary-400/50"
            :style="{
              top: dragSnappedTop + 'px',
              left: `calc(56px + (100% - 56px) * ${dragCol} / 7)`,
              width: `calc((100% - 56px) / 7)`
            }">
            <span class="hidden sm:block absolute -left-14 -top-3 w-12 text-right pr-2 text-[10px] font-mono font-medium text-primary-400">
              {{ dragTargetTime }}
            </span>
          </div>
          ...
          <div v-for="event in positionedEvents" :key="event.id"
            class="absolute rounded-lg px-1.5 py-0.5 text-[10px] overflow-hidden border-l-2 z-10 select-none transition-all duration-200 hover:shadow-md hover:z-20 hover:scale-[1.01]"
            :class="[
              getEventClass(event),
        ...
        <div v-else-if="positionedEvents.length === 0" class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none gap-3">
          ...
          <p class="text-zinc-400 dark:text-zinc-500 text-sm">本周暂无日程</p>
          <p class="text-zinc-300 dark:text-zinc-600 text-xs">点击日历创建新日程</p>
        </div>
      </div>
    </div>
  </div>
...
const { filteredEvents, selectedDate, loading, updateSchedule } = useSchedules()
...
const positionedEvents = computed(() => {
  ...
  weekDaysList.value.forEach((day, colIndex) => {
    const dayEvts = (filteredEvents.value || []).filter(e =>
